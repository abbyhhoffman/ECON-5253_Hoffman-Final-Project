...
##If this data file says error after loading, restart Rstudio to clear the cache and free MB space. 
##This helps run the large file. 

#Filter Data 

data1 <- data %>% select(-RACED,-EDUC,-EMPSTATD,-SAMPLE,-PWSTATE2,-PWMETRO,-STATEFIP,-LABFORCE)
...
est_pacificwest2 <- lm(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW + MARSTD + METROSTATUS, data = data_pacificwest)
...
vif(est_pacificwest2)


#correcting heteroskedasticity 
est_pacificwest3 <- lm_robust(logincwage ~ EDUCYRS, data = data_pacificwest)

est_pacificwest4 <- lm_robust(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW +MARSTD + METROSTATUS, data = data_pacificwest)
...
est_rockymtn2 <- lm(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW +  MARSTD + METROSTATUS, data = data_rockymtn)
...
est_rockymtn4 <- lm_robust(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW + MARSTD + METROSTATUS, data = data_rockymtn)
tidy(est_rockymtn3)
tidy(est_rockymtn4)
...
est_southwest2 <- lm(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW + MARSTD + METROSTATUS, data = data_southwest)
tidy(est_southwest)
tidy(est_southwest2)
...
est_southwest4 <- lm_robust(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW +  MARSTD + METROSTATUS, data = data_southwest)
...
est_midwest2 <- lm(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW +  MARSTD + METROSTATUS, data = data_midwest1)
...
est_midwest4 <- lm_robust(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW +  MARSTD + METROSTATUS, data = data_midwest1)
...
est_southeast2 <- lm(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW + MARSTD + METROSTATUS, data = data_southeast1)
...
est_southeast4 <- lm_robust(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW +  MARSTD + METROSTATUS, data = data_southeast1)
...
est_northeast2 <- lm(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW +  MARSTD + METROSTATUS, data = data_northeast1)
...
est_northeast4 <- lm_robust(logincwage ~ EDUCYRS + AGE + AGE.squared + SEX + RACENW +  MARSTD + METROSTATUS, data = data_northeast)
tidy(est_northeast3)
tidy(est_northeast4)
...
#goods and averaging them. Changes in the CPI are used to assess price changes associated with the 
#cost of living. The CPI is one of the most frequently used statistics for identifying periods of 
